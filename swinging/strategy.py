...
class RSIStrategy(Strategy):
    ...
    def get_exits(self, df, rsi_overbought=70):
        ...
        df.loc[bool_index, 'rsi_overbought_exit'] = 0 #1
...
class MFIStrategy(Strategy):
    ...
    def get_exits(self, df, mfi_overbought=80):
        ...
        df.loc[df.mfi >= mfi_overbought, 'mfi_overbought_exit'] = 0 #1
...
class STOStrategy(Strategy):
    ...
    def get_exits(self, df, sto_overbought=80):
        ...
        df.loc[df.sto >= sto_overbought, 'sto_overbought_exit'] = 0 #1
...
class BollingerStrategy(Strategy):
    ...
    def get_exits(self, df):
        ...
        df.loc[df.adj_close < df.bb_slim_low, 'bollinger_exit'] = 0 #1
...
class MACDStrategy(Strategy):
    ...
    def get_exits(self, df):
        df.loc[:, 'macd_exit'] = 0
        df.loc[0 > df.macd_diff, 'macd_exit'] = 0 #1
        exit_cols = ['macd_exit']
        df.loc[:, 'exits'] = df[exit_cols].sum(axis=1).clip(upper=1)
        return df
